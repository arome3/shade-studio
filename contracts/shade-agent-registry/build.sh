...
echo "Building shade-agent-registry contract..."
...
mkdir -p out
WASM_SRC="target/wasm32-unknown-unknown/release/shade_agent_registry.wasm"

if [ -f "$WASM_SRC" ]; then
  cp "$WASM_SRC" out/shade_agent_registry.wasm
elif [ -n "$(find target -name 'shade_agent_registry.wasm' -type f 2>/dev/null | head -1)" ]; then
  NEAR_WASM=$(find target -name "shade_agent_registry.wasm" -type f 2>/dev/null | head -1)
  cp "$NEAR_WASM" out/shade_agent_registry.wasm
else
...
if command -v wasm2wat &>/dev/null && command -v wat2wasm &>/dev/null; then
  ...
  BEFORE=$(wc -c < out/shade_agent_registry.wasm)
  wasm2wat out/shade_agent_registry.wasm -o out/shade_agent_registry.wat
  wat2wasm out/shade_agent_registry.wat -o out/shade_agent_registry.wasm
  rm -f out/shade_agent_registry.wat
  AFTER=$(wc -c < out/shade_agent_registry.wasm)
...
if command -v wasm-opt &>/dev/null; then
  ...
  BEFORE=$(wc -c < out/shade_agent_registry.wasm)
  wasm-opt -Oz out/shade_agent_registry.wasm -o out/shade_agent_registry.wasm
  AFTER=$(wc -c < out/shade_agent_registry.wasm)
  echo "  Before: $(human_size "$BEFORE") → After: $(human_size "$AFTER")"
else
...
SIZE=$(wc -c < out/shade_agent_registry.wasm)
HUMAN=$(human_size "$SIZE")

echo ""
echo "Build successful: out/shade_agent_registry.wasm ($HUMAN)"
